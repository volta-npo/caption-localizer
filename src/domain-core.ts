...
export function buildAdvancedDomainModel(domain, state) {
  ...
  const records = rows.map((row, index) => ({ id: row.id || `caption-${index + 1}`, label: row.label, status: row.approved ? 'approved' : row.value ? 'review' : 'missing', score: Number(row.score || 0), owner: getText('owner-reviewer', 'Volta reviewer'), evidence: row.value || 'Caption evidence required', platforms: ['Instagram', 'Facebook', 'Google Business Profile'], translationReview: row.label.toLowerCase().includes('bilingual') }));
  return { model: 'Localized caption production suite', primaryOutput: 'platform-specific captions with locale and approval QA', dashboards: ['Voice fit', 'CTA coverage', 'Translation review'], workflows: ['Tone setup', 'Local reference capture', 'Variant generation', 'Owner approval'], records, automationRules: ['Flag generic phrasing', 'Require translation reviewer for bilingual copy'], enterpriseReadiness: calc.releaseReady && approvedRows >= Math.ceil(rows.length * 0.75) };
}
